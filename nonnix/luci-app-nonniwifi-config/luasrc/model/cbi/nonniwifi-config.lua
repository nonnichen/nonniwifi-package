...
if nxfs.access('/etc/config/nonniwifi') then
	primary = uci:get_first('nonniwifi', 'global', 'primary')
	dark_primary = uci:get_first('nonniwifi', 'global', 'dark_primary')
	blur_radius = uci:get_first('nonniwifi', 'global', 'blur')
	blur_radius_dark = uci:get_first('nonniwifi', 'global', 'blur_dark')
	blur_opacity = uci:get_first('nonniwifi', 'global', 'transparency')
	blur_opacity_dark = uci:get_first('nonniwifi', 'global', 'transparency_dark')
	mode = uci:get_first('nonniwifi', 'global', 'mode')
	bing_background = uci:get_first('nonniwifi', 'global', 'bing_background')
end
...
-- [[ 模糊设置 ]]--
br = SimpleForm('config', translate('NonniWiFi Config'), translate('Here you can set the blur and transparency of the login page of nonniwifi theme, and manage the background pictures and videos.[Chrome is recommended]'))
br.reset = false
br.submit = false
...
function br.handle(self, state, data)
    if (state == FORM_VALID and data.blur ~= nil and data.blur_dark ~= nil and data.transparency ~= nil and data.transparency_dark ~= nil and data.mode ~= nil) then
        ...
        for key, value in pairs(data) do
            uci:set('nonniwifi','@global[0]',key,value)
        end 
        uci:commit('nonniwifi')
    end
    return true
end
...
sul = ful:section(SimpleSection, '', translate("Upload file to '/www/luci-static/nonniwifi/background/'"))
fu = sul:option(FileUpload, '')
fu.template = 'nonniwifi-config/other_upload'
um = sul:option(DummyValue, '', nil)
um.template = 'nonniwifi-config/other_dvalue'

local dir, fd
dir = '/www/luci-static/nonniwifi/background/'
nxfs.mkdir(dir)
http.setfilehandler(
    function(meta, chunk, eof)
        ...
        if eof and fd then
            fd:close()
            fd = nil
            um.value = translate('File saved to') .. ' "/www/luci-static/nonniwifi/background/' .. meta.file .. '"'
        end
    end
)
